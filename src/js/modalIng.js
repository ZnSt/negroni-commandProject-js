  ...
  return response
    .map(({ strIngredient, strType, strABV, strAlcohol, strDescription }) => {
        return `<div class="modalw2">
            ...
            ${strType ?`<h3 class="modalw2__title-second">${strType}</h3></div>`:``}
            <div class="modalw2__p scroll">
                ${strDescription ?`<p class="modalw2__text"><span class="modalw2__span">${strIngredient}</span> ${strDescription}</p>`:``}
                <ul class="modalw2__list">
                ${strAlcohol ?`<li class="modalw2__item">&#10038 Alcohol: ${strAlcohol}</li>`:``}
                ${strABV ?`<li class="modalw2__item">&#10038 Alcohol by volume: ${strABV}</li>`:``}
            </ul>
